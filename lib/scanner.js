...
export async function scanRepository(owner, repo, installationToken, repoId) {
  const { getInstallationOctokit } = await import('./github.js');
  ...
  if (cached && cached.length > 0) {
    await logIncident('scan', `Cache hit for repo ${owner}/${repo} at ${commitSha}`, 'info');
    return;
  }
...
${concatenated.slice(0, 50000)}`;
  ...
  await logIncident('scan', `Scan complete for ${owner}/${repo}: ${dependencies.length} dependencies found`, 'success');
}
